...
$(document).ready(function(){
    ...
    $('#btn_add').click(function(){
        $('#myModalLabel').html("Agregar Escuela  <i class='fa fa-tasks'></i>");
        $('#tag_put').remove();
        $('#btn-save').val("add");
    ...
    $(document).on('click','#btn-edit',function(){
        $('#myModalLabel').html("Editar Escuela <i class='fa fa-tasks'></i>");
        $('#schoolForm').trigger("reset");
        $('#tag_put').remove();
        $form = $('#schoolForm');
    ...
    $("#schoolForm").on('submit',function (e) {
        ...
        $(":checkbox[name=stage]").each(function () {
			if (this.checked) {
                stageArray.push($(this).attr('id'));
			}
        });
        var variable = JSON.stringify(stageArray);
        formData.append('stages',variable);
    ...
    const category={
        ...
        // console.log(dato)
                if(dato.id_status== 1){
                    statu="<span class='badge badge-success'>Activo</span>";
                    buttons += ' <a class="btn btn-sm btn-outline-info" title="Cafeterias" id="" href="'+baseUrl+'/cafeteria/'+dato.id+'"  ><i class="fa fa-coffee"></i></i></a>'
                    buttons += ' <a class="btn btn-sm btn-outline-info" title="Usuarios" id="" href="'+baseUrl+'/users/'+dato.id+'"  ><i class="icon-users"></i></a>';
                    buttons += ' <button type="button" class="btn btn-sm btn-outline-secondary open_modal" title="Editar Escuela" id="btn-edit" value="' +dato.id+'"  ><i class="fa fa-edit"></i></button>';
                    buttons += ' <button type="button" class="btn btn-sm btn-outline-danger js-sweetalert   delete-category" title="Desactivar Escuela" data-type="confirm" value="'+dato.id+'"><i class="fa fa-window-close"></i></button>';
                
                }else {
                    statu="<span class='badge badge-secondary'>Inactivo</span>";
                    buttons += ' <button type="button" class="btn btn-sm btn-outline-success delete-category" title="Activar Escuela" data-type="confirm" value="'+dato.id+'" ><i class="fa fa-check-square-o"></i></button>';
    ...
    const success = { 
                        ...
                        case 1:
                          ...
                          default:
                            ...
                            var rowNode= table.row.add( [
                                ...
                                ] ).draw().node().id = 'school_id'+dato.id;
                        
                            $('#school_id'+dato.id).css("background-color", "#c3e6cb");
                            $('#school_id'+dato.id).find('td:eq(0)').addClass("hidden-xs");
                            $('#school_id'+dato.id).find('td:eq(2)').addClass("hidden-xs");
                            ...
                            var edit = [
                                ...
                                $('#myTable').dataTable().fnUpdate(edit,$('tr#school_id'+dato.id)[0]);
                                $('#school_id'+dato.id).css("background-color", "#ffdf7e");
                            }
            ...
            deactivated:  function(data){
                console.log(data);
                if(data.id_status == 0){
                    $('#myTable').dataTable().fnDeleteRow('#school_id' + data.id);
                    ...
                    if(dato.id_status==1){
                        ...
                        $('#school_id'+dato.id).css("background-color", "#f2dede");
                    }
                }
